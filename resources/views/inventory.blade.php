            ...
            <div class="dashcode-calender">
                ...
                <div class="grid grid-cols-12 gap-2">
                    <div class="lg:col-span-3 col-span-12 card p-6">
                        <a class="btn btn-dark add-event" href="{{ route('add_form') }}" target="__blank">Tambah Inventaris</a> 
                          ...
                          <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700" id="data-table">
                              ...
                              <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                ...
                                <tr>
                                  <td class="table-td">{{$mas->no_assets}}</td>
                                  <td class="table-td ">{{$mas->nama_inventaris}}</td>
                                  <td class="table-td ">{{$mas->nama_sites}}</td>
                                  <td class="table-td ">{{$mas->jumlah_barang}}</td>
                                  <td class="table-td ">{{$mas->kondisi_sekarang}}</td>
                                  <td class="table-td ">{{$mas->tahun_beli}}</td>
                                  <td class="table-td ">{{$mas->keterangan}}</td>                    
                                  <td class="table-td ">
                                    @if($mas->audit_done == 1) <iconify-icon icon="heroicons:check-circle" width="32" height="32">{{$mas->audit_done}}</iconify-icon>
                                    @else <iconify-icon icon="heroicons:x-circle"  width="32" height="32">{{$mas->audit_done}}</iconify-icon>
                                    @endif
                                  </td>  
                                  <td class="table-td">
                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                      <a class="action-btn" href="{{route('audit_form', ['id_inv' => $mas->id] )}}" target="__blank" id="edit_inv_button" type="button"  data-inv_id="{{$mas->id}}" data-inv_no="{{$mas->no_assets}}">
...
<script>
  ...
  function refresh_table_inv(id_lokasi){
      ...
      $.ajax({
          url: '{{route("api/inventory_with_lokasi")}}', // Replace with your API endpoint
          type: 'POST',
          ...
          success: function(data) {
              ...
              if (data && data.length > 0) { // Check if data exists and is not empty
                  $.each(data, function(index, item) {
                      ...
                      newRow.append($('<td class="table-td">'+item.no_assets+'</td>'));
                      newRow.append($('<td class="table-td">'+item.nama_inventaris+'</td>'));
                      newRow.append($('<td class="table-td">'+item.nama_sites+'</td>'));
                      newRow.append($('<td class="table-td">'+item.jumlah_barang+'</td>'));
                      newRow.append($('<td class="table-td">'+item.kondisi_sekarang+'</td>'));
                      newRow.append($('<td class="table-td">'+item.tahun_beli+'</td>'));
                      newRow.append($('<td class="table-td">'+item.keterangan+'</td>'));
                      if(item.audit_done == 1){
                        newRow.append($('<td class="table-td"><iconify-icon icon="heroicons:check-circle" width="32" height="32">'+item.audit_done+'</iconify-icon></td>'));
                      } 
                      else{
                        newRow.append($('<td class="table-td"><iconify-icon icon="heroicons:x-circle" width="32" height="32">'+item.audit_done+'</iconify-icon></td>'));
                      }           
                      
                      let dupdivrow = $(''+
                      '<td class="table-td">'+
                      '<div class="flex space-x-3 rtl:space-x-reverse">'+
                        '<a class="action-btn" href="'+base_url_edit.replace(':id_inv', item.id)+'" target="__blank" id="edit_inv_button" type="button" data-inv_id="'+item.id+'" data-inv_no="'+item.no_assets+'">'+
                          '<iconify-icon icon="heroicons:pencil-square"></iconify-icon>'+
                        '</a>');
                      newRow.append(dupdivrow);
